...
function is_nucleobase(s) {
    const xs = list("A", "C", "G", "T");
    if (is_null(member(s, xs))) { //not in list xs
        return false;
    } else {
        return true;
    }
...
function find_gene_start(xs) {
    const st_cdn = xs => head(xs) === "A" && 
                            head(tail(xs)) ==="T" && 
                                head(tail(tail(xs))) === "G";
    return is_null(xs)
        ? null
        : st_cdn(xs)
        ? (is_null(tail(tail(tail(xs)))) ? list(null) : list(tail(tail(tail(xs)))))
        : find_gene_start(tail(xs));
} //returns list( list( a, g, t, c))
...
function find_gene_end(xs) {
    ...
    const stop_cdn = tga || taa || tag;
    return is_null(xs)
        ? null
        : list(stop_cdn
                ? null  
                : pair(head(xs), find_gene_start(tail(xs))));
}
...
function all_genes(xs) {
    return pair(find_gene_start(xs), find_gene_end(tail(xs)), all_genes())
}
